...
Secrets file:  .streamlit/secrets.toml with DHAN_CLIENT_ID / DHAN_ACCESS_TOKEN
"""
...
# INSTRUMENT REGISTRY
# =================================================================
# NSE index security IDs are stable and well documented -> hardcoded safely.
INDEX_REGISTRY = {
...
# SENSEX + MCX commodities roll to a NEW underlying futures contract every
# expiry cycle, so their security ids are resolved dynamically from Dhan's
# instrument master rather than hardcoded (hardcoding these WILL go stale).
...
MIN_REFRESH_SECONDS = 10  # Dhan option-chain rate limit: 1 unique request / 3s
...
def load_security_master():
    """Pull Dhan's compact instrument master (cached 6h) used to resolve
    SENSEX / commodity security ids dynamically."""
    dhan = get_dhan()
    if dhan is None:
        return pd.DataFrame()
    try:
...
def resolve_security(symbol: str):
    """Return (security_id, exchange_segment) for a symbol."""
    ...
    if not (name_col and id_col):
        st.session_state.setdefault("errors", []).append(
            ...
            f"(available columns: {list(master.columns)[:12]}...) - update column "
            "names in resolve_security() to match your master file."
        )
        return None, None
    ...
    if symbol == "SENSEX":
        ...
        # commodity: match the nearest-dated FUT contract for this symbol on MCX
...
def run_cycle(symbols):
    for i, sym in enumerate(symbols):
        ...
        if i < len(symbols) - 1:
            time.sleep(3.2)  # respect Dhan option-chain rate limit
...
with st.sidebar:
    ...
    if dhan_client is None:
        st.error("Dhan not connected \u2014 add DHAN_CLIENT_ID / DHAN_ACCESS_TOKEN to secrets.")
    else:
        st.success("Dhan client connected")

    selected = st.multiselect(
        "Instruments", ALL_SYMBOLS, default=["NIFTY", "BANKNIFTY"]
    )
    threshold = st.number_input("Spike threshold (%)", min_value=0.5, value=5.0, step=0.5)
    interval = st.slider(
        "Refresh interval (sec)",
        min_value=max(MIN_REFRESH_SECONDS, 3 * max(len(selected), 1)),
        max_value=120,
        value=max(MIN_REFRESH_SECONDS, 3 * max(len(selected), 1)),
    )
    ...
    st.markdown("---")
    st.caption(
        "SENSEX and commodity security ids are resolved dynamically each session "
        "from Dhan's instrument master since their underlying futures contract "
        "rolls over at every expiry."
    )
...
# ---------------- Ticker strip across all selected symbols ----------------
ticker_bits = []
for sym in selected:
...
st.caption(
    "Data via Dhan API option-chain endpoint (rate limited to 1 request / 3s per underlying). "
